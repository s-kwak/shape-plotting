...
merged <- data.frame(GEOID = unique(county_shp@data$GEOID)) %>%
  ...
  mutate(scaled_pop = log(pop_b+1)) %>%
  mutate(density_rank = rank(density_b, ties.method = 'random'),
         scaled_density = (density_rank/max(density_rank))) %>%
  select(GEOID, density_b, pop_b, area_b, scaled_pop, scaled_density)
...
ggplot(county_shp.df3, aes(long,lat))+ 
  geom_polygon(aes(group = group, fill = scaled_density)) +
